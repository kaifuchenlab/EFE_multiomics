...
library(foreach)
        ...
        e1e3 = ct_intmkr[cluster %in% c("EC_2","EC_3")][p_val_adj < 0.05]#[avg_log2FC > 0.5]
        
        library(clusterProfiler)
        library(org.Hs.eg.db)
        
        ece1_go <- enrichGO(gene    = e1e3[cluster == "EC_2"]$gene, # 275
                            OrgDb    = org.Hs.eg.db,
                            keyType  = 'SYMBOL',
                            ont      = "CC",
                            pAdjustMethod = "BH",
        ...
        ece3_go <- enrichGO(gene    = e1e3[cluster == "EC_3"]$gene, # 486
                            OrgDb    = org.Hs.eg.db,
                            keyType  = 'SYMBOL',
                            ont      = "CC",
                            pAdjustMethod = "BH",
        ...
        dgene[padj<0.05][cell_type == "EC_1"][log2FoldChange>0.58] # log2FC > 0.5
        ...
        EC_E_mrkr_gene[grepl("EC_3",cluster)][p_val_adj < 0.05]
        EC_E_mrkr_gene[grepl("EC_2",cluster)][p_val_adj < 0.05]
        
        #write.table(EC_E_mrkr_gene[grepl("EC_3",cluster)][p_val_adj < 0.05][,c("gene"),with=F], file = "/Users/yangyu/Desktop/EC_E3.txt", quote = F, row.names = F, col.names = F, sep = "\t")
        #write.table(EC_E_mrkr_gene[grepl("EC_2",cluster)][p_val_adj < 0.05][,c("gene"),with=F], file = "/Users/yangyu/Desktop/EC_E1.txt", quote = F, row.names = F, col.names = F, sep = "\t")
        #write.table(dgene[padj<0.05][cell_type == "EC_1"][log2FoldChange>0.58][,c("gene_name"),with=F], file = "/Users/yangyu/Desktop/EC_DE.txt", quote = F, row.names = F, col.names = F, sep = "\t")
        ...
        EC1_lisa = fread("/Users/yangyu/Desktop/Harvard_ChenLab/EP/slides/Paper/Circulation/revision/data/RevisionMinors_022326/data_results/data_IKMPresults/EC_DE_lisa_022926.csv")
        
        setnames(EC_2_lisa, c("TF","rank","P","p2","p3","p4","p5"))
        setnames(EC_3_lisa, c("TF","rank","P","p2","p3","p4","p5"))
        setnames(EC1_lisa, c("TF","rank","P","p2","p3","p4","p5"))
        
        lisa = rbind(EC_2_lisa[1:30,][,group:="EC_2_marker"],EC_3_lisa[1:30,][,group:="EC_3_marker"],EC1_lisa[1:30,][,group:="EC1_DE"])
        ...
        # order
        combined$Cell_Type <- factor(combined$Cell_Type, levels = c("EC_1","EC_2","EC_3","EndoC","FB","SMC","CM","Macrophage", "T-cell","PeriC","Neuro","Mast-cell"))
        ...
        bubble_plot <- netVisual_bubble(cellchat, sources.use = 7, targets.use = c(1,2,3), sort.by.target = T, comparison = c(1, 2), angle.x = 45, return.data = TRUE) #KEY
